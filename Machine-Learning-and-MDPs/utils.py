...
ID = 312127699  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array):
    ...
    # ====== YOUR CODE: ======
    truth_vec = np.equal(y, y_pred)
    accuracy_val = sum(truth_vec) / len(y)
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    for row in range(len(x1)):
        for col in range(len(x2)):
            dists[row][col] = np.sqrt(np.sum(np.power(np.subtract(x1[row], x2[col]), 2)))
    # ========================
    return dists
