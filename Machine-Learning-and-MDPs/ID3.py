...
class ID3:
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = 0.0
        # ====== YOUR CODE: ======
        total_len = len(right) + len(left)
        left_entropy = self.entropy(left, left_labels)
        right_entropy = self.entropy(right, right_labels)
        info_gain_value = current_uncertainty - ((len(left) / total_len) * left_entropy) - \
                          ((len(right) / total_len) * right_entropy)
        # ========================
    ...
    def partition(self, rows, labels, question: Question, current_uncertainty):
        ...
        for i, sample in enumerate(rows):
            if question.match(sample):
                ...
                false_idxs.append(i)
        true_labels = labels[true_idxs]
        true_rows = rows[true_idxs]
        false_labels = labels[false_idxs]
        false_rows = rows[false_idxs]
    ...
    def find_best_split(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        for feature_index, feature in enumerate(rows.T):
            feature_values = []
            for row in rows:
                feature_values.append(row[feature_index])
            feature_values.sort()
            for i in range(len(feature_values) - 1):
                question = Question(feature, feature_index, feature_values[i + 1])
                ...
                if gain >= best_gain:
                    best_gain, best_true_rows, best_true_labels, best_false_rows, best_false_labels, best_question = \
                        gain, true_rows, true_labels, false_rows, false_labels, question
        # ========================
    ...
    def predict_sample(self, row, node: DecisionNode or Leaf = None):
        ...
        prediction = self.predict_sample(row, node.true_branch) if node.question.match(row) \
            else self.predict_sample(row, node.false_branch)
        # ========================
    ...
    def predict(self, rows):
        ...
        root = self.tree_root
        for r in rows:
            pred = self.predict_sample(r, root)
